...
//TO-DO Add movement state of robot
boolean Moving = false;
boolean Encoders = false;
 
// Minumum and maximum values for 16-bit integers
const long int encoder_minimum = -131072;
const long int encoder_maximum = 131071;
 
// Keep track of the number of wheel ticks
volatile long int left_wheel_tick_count = 0;
volatile long int right_wheel_tick_count = 0;
...
void loop() {
 ...
 char RxedByte = 0;
  ...
  if (Serial.available()) {
    ...
    switch (RxedByte){
      case 'O': Encoders = true;
                break;
      case 'S': Encoders = false;
                break;
      default:  if (Encoders) {
                  // Record the time
